...
void main() {
  float light = dot(normalize(vertexNormal), normalize(u_reverseLightDirection));  // 只考虑漫反射
  outColor = u_color * light;
}
`;
...
export function render(canvas: HTMLCanvasElement, widthSegments = 32, heightSegments = 32) {
  ...
  const vertexNormalLocation = gl.getAttribLocation(program, "a_vertexNormal");
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    sphere.vertexNormals,
    // fGeometry,
    gl.STATIC_DRAW);
  gl.vertexAttribPointer(
    vertexNormalLocation, 3, gl.FLOAT, true, 0, 0);
  ...
  function drawScene() {
    ...
    // const instanceCount = sphere.vertexNormals.length / 3;
